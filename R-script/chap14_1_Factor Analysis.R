...
## 6. 요인축소 : 요인분석 지표 이용 요인 축소  
    ...
    ## 그냥 행렬곱으로 해도 됨!!
    cor(as.data.frame(as.matrix(subject) %*% as.matrix(loadings)))
...
###########################
    ...
    result2 <- factanal(drinking_water_df, factors = 3, rotation = "varimax",
                        scores = "regression")
    result2     # p-value is 0.0255 < 0.05 유의미 하지 않기 때문에 요인을 바꿔야 한다.  
                # but! 우리는 요인이 3개인걸 알고 시작했기 때문에 뭔가 문제가 있다고 생각하고 진행한다.!
...
## 4. 요인축소 
    ...
    factor1 <- factor_data[-4,1]
    factor2 <- factor_data[-4,2]
    factor3 <- factor_data[-4,3]
    ...
    pertinence <- numeric() # 적절성 점수가 저장될 벡터 변수 
